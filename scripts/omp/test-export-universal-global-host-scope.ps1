...
try {
    ...
    Write-TextFile -Path (Join-Path $objectRoot 'config-overlays\local-example-module-appsettings.json') -Text '{ "hostKey": "localhost", "overlayVersion": "1.0.0" }'
    Write-TextFile -Path (Join-Path $objectRoot 'config-overlays\customer-test-example-module-appsettings.json') -Text '{ "hostKey": "test-host.example.com", "overlayVersion": "1.0.0" }'
    ...
    & $objectRootExportScript -ObjectRoot $objectRoot -OutputPath $hostZip -LatestOnly -TargetHostProfile 'customer-test' | Out-Null
...
'@
    ...
    $overlaySource = Join-Path $testRoot 'customer-test-overlay.json'
    Write-TextFile -Path $overlaySource -Text '{ "hostKey": "test-host.example.com", "overlayVersion": "1.0.0" }'
