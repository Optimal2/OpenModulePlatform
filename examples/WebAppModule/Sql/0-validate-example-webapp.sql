...
WHERE COL_LENGTH(N'omp_example_webapp.ActivityLog', required.ColumnName) IS NULL;

;WITH RequiredLogIndexes(IndexName) AS
(
    SELECT v.IndexName
    FROM (VALUES (N'IX_omp_example_webapp_ActivityLog_LoggedUtc'), (N'IX_omp_example_webapp_ActivityLog_OmpUserId')) AS v(IndexName)
)
SELECT @Missing = @Missing + COUNT(1)
...
(
    ...
    WHERE idx.object_id = OBJECT_ID(N'omp_example_webapp.ActivityLog')
