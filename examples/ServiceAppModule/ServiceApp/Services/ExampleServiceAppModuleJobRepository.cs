// File: OpenModulePlatform.Service.ExampleServiceAppModule/Services/ExampleServiceAppModuleJobRepository.cs
...
public sealed class ExampleServiceAppModuleJobRepository
{
    private readonly SqlConnectionFactory _db;

    public ExampleServiceAppModuleJobRepository(SqlConnectionFactory db)
    {
        _db = db;
    }
...
(
    ...
    FROM omp_example_serviceapp.Jobs j WITH (READPAST, UPDLOCK, ROWLOCK)
    WHERE j.Status = 0
...
FROM omp_example_serviceapp.Jobs j
INNER JOIN next_job n ON n.JobId = j.JobId;";
        ...
        return new ExampleServiceAppModuleJobWorkItem
        {
    ...
    {
        const string sql = @"
UPDATE omp_example_serviceapp.Jobs
SET Status = 2,
...
INSERT INTO omp_example_serviceapp.JobExecutions(JobId, AppInstanceId, StartedUtc, FinishedUtc, Outcome, ResultJson, ErrorMessage)
