...
{
    private static string? ValidateMaintenanceSql(string sql) => OmpHostArtifactRepository.ValidateSafeModuleDefinitionSql(sql);
    ...
    {
        Assert.Contains("omp.ConfigOverlayDocuments", OmpHostArtifactRepository.ValidateSafeModuleDefinitionSql(sql)!);
    }
    ...
    {
        Assert.Contains("OMP-MODULE-SQL-CONFIG-OWNERSHIP", OmpHostArtifactRepository.ValidateSafeModuleDefinitionSql(sql)!);
    }
    ...
    public void ConfigurationOwnership_DocumentPreflightNamesModuleAndTable(string moduleKey, string sql)
    {
        ...
        var method = typeof(OmpHostArtifactRepository).GetMethod("ReadPortableSqlScripts",
    ...
    public void ConfigurationOwnership_DecodesBase64BeforeValidation()
    {
        ...
        Assert.Contains("OMP-MODULE-SQL-CONFIG-OWNERSHIP", OmpHostArtifactRepository.ValidateSafeModuleDefinitionSql(decoded!)!);
    }

    private static string? ResolvePayload(string? inlineSql, string? content, string? encoding)
    {
        var method = typeof(OmpHostArtifactRepository).GetMethod("ResolvePortableSqlText",
            System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic)!;
    ...
    {
        var error = OmpHostArtifactRepository.ValidateSafeModuleDefinitionSql(sql);
        Assert.NotNull(error);
        Assert.Contains("OMP-MODULE-SQL-CONFIG-OWNERSHIP", error);
    ...
    {
        Assert.NotNull(OmpHostArtifactRepository.ValidateSafeModuleDefinitionSql(sql));
    }
    ...
    {
        Assert.Contains(operation, OmpHostArtifactRepository.ValidateSafeModuleDefinitionSql(sql)!);
    }
...
""";
    ...
    /// </summary>
    [Fact]
    public void HostAgentValidator_AllowsMergeInsertWhoseColumnListStartsOnTheNextLine()
    {
        const string sql = """
...
""";
    ...
    [Fact]
    public void HostAgentValidator_StillBlocksMergePositionalInsert()
    {
        const string sql = """
MERGE omp.AppInstances AS target
