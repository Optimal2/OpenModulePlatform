...
END;",
    ...
    /// <summary>
    /// Reports a cleanup failure on the shared channel (<see cref="OmpTestCleanupLog"/>):
    /// Console.Error plus the log file the CI step turns into workflow warnings.
    /// </summary>
    internal static void RecordCleanupFailure(string message)
        => OmpTestCleanupLog.RecordFailure("OmpHostAgentTests", message);
