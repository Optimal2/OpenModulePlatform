...
-- Existing overlay rows are migrated by PlatformConfigurationMigration before
-- this portable schema script runs. Module SQL does not own configuration rows.
...
-- module definition is re-applied; the platform-owned migration removes duplicate
-- enabled rows before this script runs so index creation succeeds on upgrades.
IF NOT EXISTS
(
    SELECT 1
    FROM sys.indexes
