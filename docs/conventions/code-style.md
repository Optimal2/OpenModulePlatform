...
> several consumer repositories had since adopted CPM, one consumer gained
> an `.editorconfig`, a `Directory.Build.props` and a `global.json`, and two consumers moved
...
| ExampleModule (contoso-web) | `<workspace>\Contoso` | .NET |
| ExampleModule (fabrikam-web) | `<workspace>\Fabrikam` | .NET |
| ExampleModule (northwind-web) | `<workspace>\Northwind` | .NET |
| ExampleModule (adventureworks-web) | `<workspace>\AdventureWorks` | .NET |
| ExampleModule (tailwind-web) | `<workspace>\Tailwind` | .NET |
| ExampleModule (globex-web) | `<workspace>\Globex` | .NET |
...
### ExampleModule (contoso-web)

- **`.editorconfig`** — missing.
- **`Directory.Build.props`** — present, only version/company metadata (`:3-12`).
- **`Directory.Packages.props`** — present, CPM enabled (`:3`).
- **`global.json`** — pins SDK `10.0.200` (`:3-4`).
- **`.csproj`** — all projects target `net10.0` with `Nullable`/`ImplicitUsings` enabled (e.g. `Contoso.Web.csproj:3-5`). No analyzer references, no `TreatWarningsAsErrors`, no `AnalysisLevel`.

### ExampleModule (fabrikam-web)
...
### ExampleModule (northwind-web)

- **`.editorconfig`** — missing.
- **`Directory.Build.props`** — identical shape to fabrikam-web: `TargetFramework` `net10.0` (`:3`), `Nullable` `enable` (`:4`), `ImplicitUsings` `enable` (`:5`), `TreatWarningsAsErrors` `false` (`:6`).
- **`Directory.Packages.props`** — present, CPM enabled (`:3`).
- **`global.json`** — pins SDK `10.0.302` with `rollForward: latestFeature`, same as fabrikam-web.
  (Both were on that preview build at the original audit; corrected 2026-09-04.)
- **`.csproj`** — no project-level overrides found.

### ExampleModule (adventureworks-web)
...
- **`global.json`** — missing. tailwind-web is the only .NET repository in the family without an SDK
  pin, so its build floats to whatever SDK the host has installed.
- **`.csproj`** — `src/Tailwind.Web/Tailwind.Web.csproj` targets `net10.0`;
  its `PackageReference` entries are **versionless** and resolved through CPM. The test projects
  `tests/Tailwind.Web.Tests` and `tests/Tailwind.Web.UiTests` are likewise versionless;
...
### ExampleModule (globex-web)

- **`.editorconfig`** — missing.
- **`Directory.Build.props`** — present, only version/company metadata (`:3-12`).
- **`Directory.Packages.props`** — present, CPM enabled (`:3`).
- **`global.json`** — pins SDK `10.0.200` (`:3-4`).
- **`.csproj`** — all projects target `net10.0` with `Nullable`/`ImplicitUsings` enabled (e.g. `Globex.Web.csproj:3-5`). No analyzer settings, no `TreatWarningsAsErrors`, no `AnalysisLevel`.
...
| contoso-web | no | yes | enable | enable | — | default | — | — | none | 10.0.200 | — | — | — |
| fabrikam-web | no | yes | enable | enable | false | default | — | — | none | 10.0.302 | — | — | — |
| northwind-web | no | yes | enable | enable | false | default | — | — | none | 10.0.302 | — | — | — |
| adventureworks-web | yes (CRLF) | yes | enable | enable | false | latest | latest | — | none | 10.0.302 | — | — | — |
| tailwind-web | yes (CRLF) | yes | enable | enable | false | latest | latest | — | none | none | — | — | — |
| globex-web | no | yes | enable | enable | — | default | — | — | none | 10.0.200 | — | — | — |
...
1. **`.editorconfig` coverage** — OpenModulePlatform, tailwind-web, ODVGateway and adventureworks-web have one. contoso-web, fabrikam-web, northwind-web, globex-web and both JS repos lack it.
2. **Line endings** — OpenModulePlatform enforces LF; tailwind-web and ODVGateway enforce CRLF. The rest have no rule.
3. **Central Package Management** — Enabled in seven of the eight .NET repositories: OpenModulePlatform, contoso-web, fabrikam-web, northwind-web, globex-web, adventureworks-web and tailwind-web. **ODVGateway is the only one without it**, which is also the mechanism behind its package-pin drift: with no `Directory.Packages.props` there is no single place to lift a version.
4. **Warning/error handling** — fabrikam-web, northwind-web, tailwind-web, ODVGateway and adventureworks-web explicitly set `TreatWarningsAsErrors` to `false`. OpenModulePlatform, contoso-web and globex-web do not set it at all. None treat warnings as errors.
5. **Static analysis** — `AnalysisLevel` is set in tailwind-web, adventureworks-web and ODVGateway (all `latest`). `EnforceCodeStyleInBuild` is set only in ODVGateway (`true`).
6. **SDK pinning** — Seven of eight .NET repositories pin an SDK in `global.json`, all with `rollForward: latestFeature`: OpenModulePlatform `10.0.400`, ODVGateway `10.0.300`, fabrikam-web/northwind-web/adventureworks-web `10.0.302`, contoso-web/globex-web `10.0.200`. **tailwind-web is the only repository with no pin** and therefore floats to the host SDK. The pins span four feature bands; today they all roll forward to the same installed SDK, so the spread is latent rather than active.
