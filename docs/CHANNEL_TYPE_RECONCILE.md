...
requirements. Contoso is the reference implementation, commit
`7a2deecfe26872c455618c3355c54611c3efd447`, specifically
`sql/1-setup-contoso.sql:229-332`.

All source `file:line` citations refer to that Contoso commit unless explicitly
...
channel/config-version FK. Sources: `sql/1-setup-contoso.sql:734-736`, `:791-798`,
`:842-875`. The setup also defines consistency triggers; inspect their complete
bodies when copying the table setup (`sql/1-setup-contoso.sql:879-939`).
...
2. Choose an enabled version of the same channel type. With a non-null pin, only that
   ...
   Source: `sql/1-setup-contoso.sql:271-279`, `:294-295`.
3. A concrete placement uses `COALESCE(ai.HostId, wi.HostId)` (app host first).
   ...
   Source: `sql/1-setup-contoso.sql:269-270`, `:280-295`.
4. A tuple is active exactly when `Channels.IsEnabled = 1 AND DesiredState = 1`.
   ...
   disables the channel in the repository. Sources: `sql/1-setup-contoso.sql:261-295`;
   `Contoso.Runtime/Services/ContosoRepository.cs:3416-3423`.
...
Source: `sql/1-setup-contoso.sql:233-332`.

| Object | Reads | Writes | Source |
| --- | --- | --- | --- |
| `omp_<module>.Channels` | `ChannelId`, `AppInstanceId`, `WorkerInstanceId`, `ChannelTypeId`, `PinnedChannelTypeVersionId`, `IsEnabled`, `DesiredState` | None | `sql/1-setup-contoso.sql:261-278` |
| `omp_<module>.ChannelTypeVersions` | `ChannelTypeId`, `ChannelTypeVersionId`, `ArtifactId`, `IsEnabled`, `IsDefault` | None | `sql/1-setup-contoso.sql:273-278` |
| `omp.AppInstances` | `AppInstanceId`, `HostId`, `TargetHostTemplateId` | None | `sql/1-setup-contoso.sql:269`, `:282-291` |
| `omp.WorkerInstances` | `WorkerInstanceId`, `HostId` | None | `sql/1-setup-contoso.sql:270`, `:282-289` |
| `omp.HostDeploymentAssignments` | `HostId`, `HostTemplateId`, `IsActive` | None | `sql/1-setup-contoso.sql:287-292` |
| `omp.HostArtifactRequirements` | `RequirementKey`, `HostId`, `ArtifactId`, `IsEnabled` | Update: `IsEnabled`, `UpdatedUtc`, `ArtifactId`; insert: `HostId`, `ArtifactId`, `RequirementKey`, `IsEnabled`, `CreatedUtc`, `UpdatedUtc` | `sql/1-setup-contoso.sql:300-330` |
| `@Effective` (table variable) | `RequirementKey`, `HostId`, `ArtifactId`, `IsActive` | Insert: `ChannelId`, `HostId`, `ArtifactId`, `RequirementKey`, `IsActive` | `sql/1-setup-contoso.sql:252-267`, `:307-324` |
...
Keep those identities distinct. Sources: `sql/1-setup-contoso.sql:266`, `:319-330`;
`Contoso.Runtime/Services/ContosoRepository.cs:3302-3327`.
...
and protect referenced artifacts even after a requirement is disabled. Contoso
provides that FK (`sql/1-setup-contoso.sql:223`). OMP evidence at the baseline above:
...
(`Contoso.Runtime/Services/ContosoWorkerEngine.cs:60-117`); see
[HostAgent's existing RPC](HOST_AGENT.md#implemented). Placements without any host
mapping produce no rows in the SQL procedure (`sql/1-setup-contoso.sql:280-295`).
...
`Contoso.Runtime/Services/ContosoRepository.cs:2787-2794`.

| Trigger | Web/runtime entry and exact downstream call site | Mechanism |
| --- | --- | --- |
| Save channel, including placement, pin and state edits | `Contoso.Web/Pages/Channels/Index.cshtml.cs:413`; `Contoso.Runtime/Services/ContosoRepository.cs:2568` | `SaveChannelAsync` calls the wrapper after save and permission sync. Its transaction also writes the saved channel's requirement directly at `Contoso.Runtime/Services/ContosoRepository.cs:2519-2538`; keep the later global reconcile for complete placement expansion. |
| Enable or disable channel | `Contoso.Web/Pages/Channels/Index.cshtml.cs:306`; `Contoso.Runtime/Services/ContosoRepository.cs:2684` | `SetChannelEnabledAsync` calls the wrapper after the mutation. |
| Start or stop desired state | `Contoso.Web/Pages/Channels/Index.cshtml.cs:332`; `Contoso.Runtime/Services/ContosoRepository.cs:2784` | `SetChannelDesiredStateAsync` calls the wrapper after the mutation. |
| Delete channel | `Contoso.Web/Pages/Channels/Index.cshtml.cs:461`; `Contoso.Runtime/Services/ContosoRepository.cs:3400`, `:3491` | `DeleteChannelAsync` directly deletes requirements for the exact channel key in the deletion transaction. Soft deletion disables/stops the channel (`:3416-3423`); hard deletion removes it (`:3487`). No procedure call occurs before the return (`:3528-3539`). |
| Save channel-type version, change artifact, enable flag or default | `Contoso.Web/Pages/Channels/Index.cshtml.cs:671-682`; `Contoso.Runtime/Services/ContosoRepository.cs:3707` | `SaveChannelTypeVersionAsync` calls the wrapper. `IsDefault` is part of this save; existing default flags are cleared at `Contoso.Runtime/Services/ContosoRepository.cs:3666-3670`. |
| Delete channel-type version, including default reassignment | `Contoso.Web/Pages/Channels/Index.cshtml.cs:718`; `Contoso.Runtime/Services/ContosoRepository.cs:3776` | `DeleteChannelTypeVersionAsync` calls the wrapper. Pinned/last-version deletion is rejected (`:3727-3736`); default replacement happens inside the transaction (`:3746-3768`). |
| Portal Reconcile button | `Contoso.Web/Pages/Channels/Index.cshtml.cs:512-523` | Admin-guarded direct wrapper call. The displayed count is prefix rows updated since five seconds before the call (`:522-536`), not a count returned by the procedure or proof of artifact deployment. |
| Recover unloadable pins | `Contoso.Runtime/Services/ChannelTypeVersionReconciliationService.cs:164`; `Contoso.Runtime/Services/ContosoRepository.cs:3043-3044` | `RecoverUnloadableChannelTypePinsAsync` executes the procedure after commit only when `@recovered > 0`. |
| Advance default to registered artifact | `Contoso.Runtime/Services/ChannelTypeVersionReconciliationService.cs:72`; `Contoso.Runtime/Services/ContosoRepository.cs:3331` | `AdvanceChannelTypeDefaultVersionAsync` executes the procedure after its transaction commits. |
| Worker start and interval | `Contoso.Runtime/Services/ContosoWorkerEngine.cs:201`, `:928`; injected by `Contoso.Worker/ContosoWorkerFactory.cs:30` | Starts the version-reconciliation loop beside the channel loop. First pass precedes the timer wait; disabled when the service is absent or interval is nonpositive (`Contoso.Runtime/Services/ContosoWorkerEngine.cs:914-939`). Requirements are reconciled through recovery/advance above, not unconditionally on every worker start. |
| Web startup and interval | `Contoso.Web/Program.cs:32`; `Contoso.Web/Services/ChannelTypeVersionReconciliationHostedService.cs:39` | Registered hosted service invokes version reconciliation when enabled, then waits for its configured interval (`Contoso.Web/Services/ChannelTypeVersionReconciliationHostedService.cs:30-61`). |
| Initialization SQL, before and after final channel version mapping | `sql/2-initialize-contoso.sql:421`, `:1575` | Two explicit procedure executions; preserve the final pass after pin/default mapping. |
...
Sources: `Contoso.Runtime/Services/ChannelTypeVersionReconciliationService.cs:34-64`,
`:119`, `:137-164`;
`Contoso.Tests/Integration/ChannelTypeVersionReconciliationIntegrationTests.cs:158-173`.
...
wrapper calls at `Contoso.Tests/ReconcileChannelTypeArtifactRequirementsTests.cs:333`,
`:347`, `:360`, `:373`; version-service calls at
`Contoso.Tests/Integration/ChannelTypeVersionReconciliationIntegrationTests.cs:46`,
`:107`, `:119`, `:126`, `:160`, `:171`, `:222`, `:278`; and the simulated-import SQL
procedure execution at `Contoso.Tests/Integration/ChannelTypeVersionReconciliationIntegrationTests.cs:374`.
The procedure-creation stub itself is DDL, not reconciliation
(`sql/1-setup-contoso.sql:233-237`).
...
- Declare **every** setup `CREATE TABLE` in `integrity.requiredTables`, with its own
  ...
  `contoso.module-definition.json:129`. The reference has 22 entries at
  `contoso.module-definition.json:133-266`: `ValidationLists`, `MappingSets`,
...
- Preserve `integrity.requiredModuleRows.channelTypes` entries with
  ...
  channel type, `file_drop_to_contoso`; do not claim every plugin appears in that array.
  Source: `contoso.module-definition.json:399-414`.
- Declare the worker and its channel-type artifacts together in
  ...
  channel-type artifacts. Source: `contoso.module-definition.json:66-88`.
- Set `allowMultipleActiveInstances: true` on the worker app declaration, as in
  `contoso.module-definition.json:23-29`. Preserve per-worker channel lease
  identity (`sql/1-setup-contoso.sql:964-974`).
...
evidence (`Contoso.Tests/ReconcileChannelTypeArtifactRequirementsTests.cs:327-373`;
`Contoso.Tests/Integration/ChannelTypeVersionReconciliationIntegrationTests.cs:25-30`).

| Reference test and scenario | Source |
| --- | --- |
| `Reconcile_UpsertsRequirement_ForEnabledRunningChannel`: correct an existing requirement to the effective artifact and enable it. | `Contoso.Tests/ReconcileChannelTypeArtifactRequirementsTests.cs:328-337` |
| `Reconcile_DisablesRequirements_ForDisabledChannel`: retained row becomes disabled. | `Contoso.Tests/ReconcileChannelTypeArtifactRequirementsTests.cs:341-350` |
| `Reconcile_DisablesRequirements_ForStoppedDesiredState`: retained row becomes disabled. | `Contoso.Tests/ReconcileChannelTypeArtifactRequirementsTests.cs:354-363` |
| `Reconcile_UpsertsNewRequirement_WhenPinnedVersionChanges`: requirement follows the new pinned artifact. | `Contoso.Tests/ReconcileChannelTypeArtifactRequirementsTests.cs:367-377` |
| `Reconcile_AdvancesDefault_MovesFollowers_KeepsPins_RetiresRows_RepointsRequirements`: move followers and linked config, preserve the eligible explicit older pin, copy schema/loader identity, retire the old default, move baseline requirements, and return zero on a second pass. | `Contoso.Tests/Integration/ChannelTypeVersionReconciliationIntegrationTests.cs:37-107` |
| `Reconcile_WaitsForTheMatchingWorkerArtifact`: channel-type arrival alone is insufficient when the app ships workers; matching worker arrival permits advance. | `Contoso.Tests/Integration/ChannelTypeVersionReconciliationIntegrationTests.cs:111-129` |
| `Reconcile_RecoversAPinTheWorkerCannotLoad`: recover an incompatible pin even without a default advance; repeat safely. | `Contoso.Tests/Integration/ChannelTypeVersionReconciliationIntegrationTests.cs:148-173` |
| `Reconcile_RecoversAPinTheWorkerCannotLoad_EvenWithNoAdvanceCandidates`: prove the candidate list is empty and recovery still occurs. | `Contoso.Tests/Integration/ChannelTypeVersionReconciliationIntegrationTests.cs:197-230` |
| `Reconcile_PinRecovery_RepointsPerChannelRequirements_AcrossTwoConsecutiveImports`: simulate two imports; assert a single enabled artifact version and the new per-channel artifact after each. | `Contoso.Tests/Integration/ChannelTypeVersionReconciliationIntegrationTests.cs:263-302` |
| `FileDropSeedSchemaMatchesTheFactorySchema` and `ImageComposeSeedSchemaMatchesTheFactorySchema`: adapt one test per plugin; compare factory/seed JSON canonically and reject duplicate properties in either copy. These are parity unit tests. | `Contoso.Tests/ChannelTypeSchemaParityTests.cs:25-59`, `:68-88` |
...
- [ ] Verify the module-owned table shapes, artifact FK, configuration relationships
  and composite lease key against `sql/1-setup-contoso.sql:189-225`, `:336-429`,
...
- [ ] Inventory all setup `CREATE TABLE` statements and compare their names with
  ...
  `contoso.module-definition.json:23-29`, `:66-88`, `:129-266`, `:399-414`.
- [ ] Map each production row in the call-site inventory to the adopting module,
  ...
  `Contoso.Runtime/Services/ContosoRepository.cs:2568`, `:2684`, `:2784`,
  `:3044`, `:3331`, `:3491`, `:3707`, `:3776`;
  `Contoso.Web/Pages/Channels/Index.cshtml.cs:523`;
  `Contoso.Runtime/Services/ContosoWorkerEngine.cs:201`, `:928`;
  `Contoso.Web/Services/ChannelTypeVersionReconciliationHostedService.cs:39`;
  `sql/2-initialize-contoso.sql:421`, `:1575`.
- [ ] Execute the nine named database scenarios and the two schema-parity scenarios
  ...
  `Contoso.Tests/ReconcileChannelTypeArtifactRequirementsTests.cs:327-377`;
  `Contoso.Tests/Integration/ChannelTypeVersionReconciliationIntegrationTests.cs:25-302`;
  `Contoso.Tests/ChannelTypeSchemaParityTests.cs:25-59`.
- [ ] Compare concrete-host, role-assignment and stale-row results with the SQL
  ...
  Source: `sql/1-setup-contoso.sql:261-330`.
- [ ] Check per-channel and baseline keys separately after two successive imports;
  ...
  `Contoso.Tests/Integration/ChannelTypeVersionReconciliationIntegrationTests.cs:263-302`;
  `Contoso.Web/Pages/Channels/Index.cshtml.cs:522-544`.
...
- A mandatory unconditional stored-procedure call on channel deletion or every
  ...
  `Contoso.Runtime/Services/ContosoRepository.cs:3491`, `:3528-3539`;
  `Contoso.Runtime/Services/ContosoWorkerEngine.cs:201`, `:914-939`;
  `Contoso.Runtime/Services/ChannelTypeVersionReconciliationService.cs:34-76`.
- Named integration coverage for role expansion, stale host-assignment removal,
  ...
  is inspectable at `sql/1-setup-contoso.sql:261-330`; the named test evidence is
  `Contoso.Tests/ReconcileChannelTypeArtifactRequirementsTests.cs:327-377` and
  `Contoso.Tests/Integration/ChannelTypeVersionReconciliationIntegrationTests.cs:36-302`.
- A new host-neutral RPC protocol and a blanket retention guarantee based on the
  ...
  (`sql/1-setup-contoso.sql:280-295`), and the existing worker RPC call is at
  `Contoso.Runtime/Services/ContosoWorkerEngine.cs:104-117`; the separately
