...
OpenModulePlatformHostAgentFirst-<version>/
  ...
  configs/
    linus.json
    alfons.json
    customer-production-1825.json
    customer-production-1826.json
  data/
    global/
      artifacts/
...
2. Put one or more environment-specific bootstrap JSON files in `configs`.
   Keeping names such as `linus.json`, `alfons.json`, `customer-production.json`,
   and `customer-test.json` lets one package carry multiple installation profiles.
   ...
   {
     "profile": {
       "displayName": "the customer Production - host-a",
       "machineNames": [ "host-a", "host-a.example.com" ]
     }
   }
   ```
...
## the customer And Other Customer Packages
...
HostAgentFirst = @{
    ...
    AdditionalSqlFiles = @(
        ...
        @{ Source = 'customer-prod-bootstrap.sql'; Destination = 'Customer\customer-prod-bootstrap.sql' }
    )
...
For example, a protected the customer package can set:
