    ...
    private sealed class InstallerForm : Form
    {
            ...
            bool quickMode = true)
        {
            ...
            foreach (var script in _config.Sql.Scripts.Where(static script => script.Enabled))
            {
                ...
                {
                    // bootstrap-local.sql is generated by package-hostagent-first.ps1
                    // at packaging time and never exists in the source repositories,
                    // so a missing source is expected here rather than a problem.
                    if (scriptFileName.Equals("bootstrap-local.sql", StringComparison.OrdinalIgnoreCase))
                    {
                        lines.Add($"  OK      {script.Path}: generated at packaging time; no source to sync in this run.");
                        unchanged++;
                    }
                    else
                    {
