...
SELECT @@ROWCOUNT;";
    ...
    /// <summary>
    /// Replaces the package-delivered configuration rows AND carries operator
    /// edits forward from the previous version in ONE transaction. The two used
    /// to be separate committed round-trips, so a crash between them left the new
    /// artifact registered with pristine rows while a re-import took the
    /// existing-identity path that never carries forward — dropping the previous
    /// version's operator edits permanently (R4-D4). Used by the new-registration
    /// import paths; the existing-identity re-import path keeps plain replace so
    /// carry-forward is never re-applied over a deliberate revert.
    /// </summary>
        ...
        CancellationToken ct)
    {
        ...
        await using (var carryForward = new SqlCommand(
            ...
            tx))
        {
            carryForward.Parameters.AddWithValue("@ArtifactId", artifactId);
    ...
    private static async Task ReplaceArtifactConfigurationFilesCoreAsync(
        SqlConnection conn,
        SqlTransaction tx,
        int artifactId,
        IReadOnlyList<ArtifactPackageConfigurationFile> configurationFiles,
...
WHERE ModuleDefinitionDocumentId = @moduleDefinitionDocumentId;";
    ...
    private static IReadOnlyList<RequiredDatabaseObject> ReadRequiredDatabaseObjects(string definitionJson)
    {
        ...
        // module packages and HostAgent are deployed independently, so a forward-compatible
    ...
    /// read-only validation script. An empty result is what lets an import skip the
    /// definition, so anything this method cannot verify is reported as missing (R12-G3).
    /// </summary>
        ...
        CancellationToken ct)
    {
        ...
        if (validationSignal is not null)
        {
            ...
            // every idempotent repair script is re-run -- the same answer the unfiltered
            // folder-import repair path already gives when validation reports unhealthy.
            // Scripts that are not idempotent are left out on purpose: the filtered repair
            // path throws on them, which would turn a heal into a failed import.
        ...
        CancellationToken ct)
    {
        foreach (var script in scripts.Where(IsValidationScript))
        {
            ...
            // those cases fall through above; the unfiltered repair path already rejects the
            // definition loudly for exactly those reasons.
            try
            {
