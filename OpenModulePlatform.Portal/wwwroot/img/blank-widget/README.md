...
Portal admins can upload custom blank-widget media from the widget in normal
view mode by opening its image library button. Dashboard edit mode is not needed
for library maintenance. A ZIP import can include `.gif`, `.png`, `.jpg`, or
...
database and prefer `binaryDataHash` references when ids are not known yet, so
the package can be moved between installations without storing custom media in
the Portal web artifact.
