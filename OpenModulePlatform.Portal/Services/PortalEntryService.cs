...
public sealed class PortalEntryService
{
    ...
    private const string SyncDefaultPortalEntriesSql = @"
;WITH app_entries AS
(
...
(
    ...
    FROM (VALUES
        ...
        (N'omp_portal', N'portal:admin-host-resources', N'Host resources', N'Latest host CPU and memory telemetry.', N'/admin/hostresources', 75),
...
WHERE entry_key = N'portal:admin-dashboard-widgets';";
    ...
    public async Task ResetDefaultEntriesAsync(CancellationToken ct)
    {
        await using var conn = _db.Create();
        await conn.OpenAsync(ct);

        await using var cmd = new SqlCommand(SyncDefaultPortalEntriesSql, conn);
...
WHERE portal_entry_id = @portal_entry_id;";
    ...
    public async Task<PortalEntryDeleteResult> DeleteAsync(int portalEntryId, CancellationToken ct)
    {
        await using var conn = _db.Create();
        await conn.OpenAsync(ct);
        await using var tx = (SqlTransaction)await conn.BeginTransactionAsync(ct);
...
WHERE portal_entry_id = @portal_entry_id;";
            ...
            await using (var selectCmd = new SqlCommand(selectSql, conn, tx))
            {
                ...
                {
                    await tx.RollbackAsync(ct);
                    return PortalEntryDeleteResult.NotFound;
                }
            ...
            {
                await tx.RollbackAsync(ct);
                return PortalEntryDeleteResult.NotAllowed;
            }
...
WHERE parent_entry_id = @portal_entry_id;";
            ...
            await tx.CommitAsync(ct);
            return PortalEntryDeleteResult.Deleted;
        }
        catch
        {
            await tx.RollbackAsync(ct);
