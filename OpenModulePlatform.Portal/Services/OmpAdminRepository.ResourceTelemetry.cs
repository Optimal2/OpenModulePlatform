...
ORDER BY latest.SampleKey;";
        ...
        while (await rdr.ReadAsync(ct))
        {
            rows.Add(new HostResourceLatestRow
            {
                ...
                SampleKey = rdr.IsDBNull(3) ? string.Empty : rdr.GetString(3),
                SampleValue = rdr.IsDBNull(4) ? 0 : rdr.GetDouble(4),
                SampleCount = rdr.IsDBNull(5) ? 0 : rdr.GetInt32(5),
                FirstSampledUtc = rdr.IsDBNull(6) ? DateTime.MinValue : rdr.GetDateTime(6),
                LastSampledUtc = rdr.IsDBNull(7) ? DateTime.MinValue : rdr.GetDateTime(7),
                MinValue = rdr.IsDBNull(8) ? null : rdr.GetDouble(8),
                MaxValue = rdr.IsDBNull(9) ? null : rdr.GetDouble(9)
            });
        }
