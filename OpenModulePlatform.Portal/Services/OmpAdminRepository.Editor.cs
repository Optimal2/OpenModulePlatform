...
WHERE ModuleKey = @ModuleKey;";
        ...
        CancellationToken ct)
    {
        ...
        foreach (var script in scripts.Where(script => !IsValidationScript(script) && repairableKeys.Contains(script.Key)))
        {
            ...
            try
            {
                await OpenModulePlatform.ModuleDefinitions.PlatformConfigurationMigration.ApplyForDefinitionAsync(conn, definition.DefinitionJson, script.Key, ct);
  ...
  AND ar.IsEnabled = 1;";
        ...
        CancellationToken ct)
    {
        ...
        foreach (var stalePath in existingPaths.Where(path => !incomingPaths.Contains(path)))
        {
            ...
            Add(delete, "@RelativePath", stalePath);
        ...
        foreach (var configurationFile in configurationFiles)
        {
            await using var upsert = new SqlCommand(
                ...
                conn,
                tx);
            Add(upsert, "@ArtifactId", artifactId);
            Add(upsert, "@RelativePath", configurationFile.RelativePath);
