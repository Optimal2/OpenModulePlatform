...
/// <summary>
/// Edits a module definition.
/// Definitions stay environment-neutral; runtime and placement details belong to instance-level rows.
/// </summary>
public sealed class ModuleEditModel : OmpPortalPageModel
{
    ...
    public async Task<IActionResult> OnGet(int? id, CancellationToken ct)
    {
        ...
        SetTitles(id.HasValue ? "Edit module" : "Create module");

        if (!id.HasValue)
        {
            Input.ModuleType = "WebAppModule";
            Input.IsEnabled = true;
            return Page();
        }

        var row = await _repo.GetModuleAsync(id.Value, ct);
        if (row is null)
        {
            return NotFound();
        }
    ...
    public async Task<IActionResult> OnPost(CancellationToken ct)
    {
        ...
        SetTitles(IsCreate ? "Create module" : "Edit module");
        ...
        try
        {
            var id = await _repo.SaveModuleAsync(
                new ModuleEditData
                {
            ...
            StatusMessage = IsCreate ? T("Module created.") : T("Module updated.");
            return RedirectToPage("/Admin/ModuleEdit", new { id });
        }
    ...
    public async Task<IActionResult> OnPostDelete(CancellationToken ct)
    {
        ...
        try
        {
            await _repo.DeleteModuleAsync(Input.ModuleId, ct);
            StatusMessage = T("Module deleted.");
            return RedirectToPage("/Admin/Modules");
        }
        catch (SqlException ex)
        {
            ModelState.AddModelError(
    ...
    private static string ToFriendlySqlMessage(SqlException ex, string fallback)
        => PortalTextLocalizer.DescribeSqlError(ex, fallback, "A module with the same key already exists.");
